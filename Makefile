...
all: clean
	...
	mkdir --parents $(PWD)/build/Boilerplate.AppDir/gpsprune
	apprepo --destination=$(PWD)/build appdir boilerplate 

	wget --output-document=$(PWD)/build/Boilerplate.AppDir/gpsprune/gpsprune.jar https://activityworkshop.net/software/gpsprune/gpsprune_20.1.jar
	...
	echo '' >> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo '' >> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo "LD_LIBRARY_PATH=\$${LD_LIBRARY_PATH}:\$${APPDIR}/gpsprune" >> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo "export LD_LIBRARY_PATH=\$${LD_LIBRARY_PATH}" >> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo '' >> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo '' >> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo 'exec $${APPDIR}/jre/bin/java -Xmx1024M -jar $${APPDIR}/gpsprune/gpsprune.jar "$${@}"' >> $(PWD)/build/Boilerplate.AppDir/AppRun
	...
	export ARCH=x86_64 && $(PWD)/bin/appimagetool.AppImage $(PWD)/build/Boilerplate.AppDir $(PWD)/GpsPrune.AppImage
	chmod +x $(PWD)/GpsPrune.AppImage
